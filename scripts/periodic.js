...
PropertiesContainer.style.width = `${document.querySelector(".Table").offsetWidth}px`;
document.querySelector("column").style.width = `${document.querySelector(".Table").offsetWidth}px`;
    ...
    Array.from(Elements).forEach(Element => {
        ["mouseenter", "click"].forEach(Event => {
            Element.addEventListener(Event, () => {
                ...
                PropertiesContainer.querySelector(".Element").querySelector("header").innerHTML = ElementSymbol;
                ...
                InspectSpans[2].innerHTML = `Period: ${GetPeriod(ProtonCount)}`;
                PropertiesContainer.querySelector(".Element").style.backgroundColor = Types[Element.className] || Types.Metal;

                PropertiesContainer.querySelector(".Showcase").querySelector(".Electrons").innerHTML = "";

                let ElectronCount = 0;
                while (ElectronCount < ProtonCount) {
                	const Charge = document.createElement("div");
                	const MaxElectrons = ElectronCount < 2 ? 2 : 8;
                
                	for (let Index = 0; Index < MaxElectrons && ElectronCount < ProtonCount; Index++) {
                		const Electron = document.createElement("div");
                		Charge.appendChild(Electron);
                		ElectronCount++;
                	}
                
                	PropertiesContainer.querySelector(".Showcase").querySelector(".Electrons").appendChild(Charge);
                }
            });
        });
    });
...
Array.from(PeriodicTable.children).forEach(Group => {
