...
Array.from(PeriodicTable.children).forEach(Group => {
    ...
    if (TransitionGroup) {
        Array.from(Group.children).forEach(Subgroup => AddElementListeners(Subgroup.children, GroupNumber, GroupName));
    } else {
        AddElementListeners(Group.children, GroupNumber, GroupName);
    }
